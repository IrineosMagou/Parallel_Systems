...
readonly MPI_SOURCE="monte_carlo.c"
...
PROC_COUNT=(1)
for((p=2; p<=MAX_NPROCS; p+=PROC_STEP)); do
    PROC_COUNT+=($p)
done
...
function compile_program { 
    echo "======================================================"
    echo " --> Compiling: $MPI_SOURCE.c with $HELPERS_SRC.c"
    echo "======================================================"

# Compile the MPI program using mpicc
    mpicc -o "$EXECUTABLE" "$MPI_SOURCE" "$HELPERS_SRC"
...
function run_and_average {
    ...
    for((r=1; r<RUNS_PER_NPROCS; r+=1)); do
        ...
        output=$(mpirun -np "$processes" ./"$EXECUTABLE" "$TOTAL_THROWS")
        current_time=$(echo "$output" | awk '/MPI Execution Time/{printf "%.6f", $4}')
        
        if [[ -z "$current_time" ]]; then
            echo "Warning: Could not extract time on run ${j} for processes=${processes}. Skipping this run."
            continue
        fi
...
# --- Input Validation ---
if [ $# -ne 1 ]; then
    echo "Usage: $0 <Total_Number_Of_Throws>"
    echo "Example: $0 100000000"
    exit 1
fi
